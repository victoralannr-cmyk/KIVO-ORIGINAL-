...
vec3 background_color(vec2 uv) {
    ...
    vec3 bg_col = mix(vec3(0.098, 0.137, 0.227), vec3(0.682, 0.717, 0.792), smoothstep(0.2, 1.2, d)); // #19233A to #AEB7C4
...
vec3 getLineColor(float t, vec3 baseColor) {
  ...
  float m = uv.y - y;
  float glow = 0.01 / (abs(m) + 0.005);
  return glow * glow * 0.8;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 b = background_color(gl_FragCoord.xy / iResolution.xy);
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      lineGradient: {
        value: Array.from({ length: MAX_GRADIENT_STOPS }, () => new Color(1, 1, 1))
      },
